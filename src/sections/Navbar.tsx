...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      <div
        className={`transition-[background-color,border-color,backdrop-filter] duration-500 ease-premium ${
          scrolled || open
      ...
      >
        ...
        >
          <a
            href="#inicio"
            aria-label="INTELRA — início"
            className="relative z-10 justify-self-start text-[0.95rem] text-bone"
          ...
          <ul className="hidden items-center gap-1 lg:flex">
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="rounded-full px-4 py-2 text-[0.875rem] text-bone/65 transition-colors duration-300 hover:bg-white/[0.05] hover:text-bone"
                >
                  {link.label}
                </a>
              </li>
          ...
          <div className="flex items-center justify-self-end gap-2">
            <span className="hidden sm:block">
              <CtaButton className="h-10! px-4! text-[0.85rem]!" icon="arrow">
                Falar com a INTELRA
              </CtaButton>
            </span>
            <button
      ...
      <AnimatePresence>
        {open && (
          <m.div
            id="mobile-menu"
            className="fixed inset-0 top-16 z-40 flex flex-col bg-ink-950/95 backdrop-blur-2xl lg:hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
              ...
              {NAV_LINKS.map((link, i) => (
                ...
                >
                  <a
                    href={link.href}
                    onClick={() => setOpen(false)}
                    className="group flex items-center justify-between border-b border-white/[0.07] py-5 text-3xl font-medium tracking-[-0.03em] text-bone/85 hover:text-bone"
                  >
                    {link.label}
                    <ArrowUpRight className="size-5 text-mute transition-transform duration-500 group-hover:rotate-45 group-hover:text-gold" />
                  </a>
                </m.li>
              ))}
            </ul>
