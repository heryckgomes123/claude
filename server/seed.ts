...
export async function seed(q: Queryable, opts: { now?: Date } = {}) {
  ...
  const simulate = (at: Date, kind: 'table' | 'bot', table: STable | null, humans: SUser[], botCount: number, difficulty: BotDifficulty) => {
    ...
    if (kind === 'table') {
      ...
      activities.push(['game_win', state.players[state.winners[0]].userId, table?.club?.id ?? null, s.prize > 0 ? `${w} venceu em ${roomName} (+${s.prize} Miúdas).` : `${w} venceu em ${roomName}.`, J({ gameId, prize: s.prize }), realFinish]);
    }
    };
  };
